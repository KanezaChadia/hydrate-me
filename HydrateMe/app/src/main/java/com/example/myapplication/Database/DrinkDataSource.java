...
public class DrinkDataSource {
   ...
   public DateLog createDateLog(int amount,int n,String d) {
    ...
    public boolean updateConsumedWaterForTodayDateLog( int amount ) {
       int consumed = geConsumedWaterForTodayDateLog() + amount;
        ContentValues cv = new ContentValues();
    ...
    @SuppressLint("Range")
    public int geConsumedWaterForTodayDateLog() {
        int waterAmount = 0;
        Cursor cursor = database.query(DrinkDbHelper.Date_TABLE_NAME,
                new String[]{DrinkDbHelper.COLUMN_ID,
                        DrinkDbHelper.COLUMN_WATER_DRUNK, DrinkDbHelper.COLUMN_WATER_NEED},
                null, null, null, null, DrinkDbHelper.COLUMN_ID
                        + " DESC", " 1");
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
    ...
    public int getConsumedPercentage() {
        ...
        if(waterNeed==0)
            return 0;
         int value= geConsumedWaterForTodayDateLog()*100/waterNeed;
           if (value>100)
               return 100;
        return value;
    }
